...
| **SCSVS&nbsp;VR&nbsp;ID** | 要件                                                                 | L1 | L2 | L3 | SWE |
| ------------------------- | -------------------------------------------------------------------- | -- | -- | -- | --- |
| S4.1.C1      | 認可には msg.sender (tx.origin ではない) を使用して、悪意のある仲介コントラクトによるフィッシングを避けます。オプションで、require(tx.origin == msg.sender) は送信者を EOA のみに制限します。EOA のみのアクセスが明示的に必要とされる場合にのみ使用します。 |    | ✓  | ✓  |     |
